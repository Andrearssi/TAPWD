    ...
    if (!$_SESSION['status']) {
        ...
        <header>
            <nav class="navbar navbar-light navbar-expand-md navwhite">
                <div class="container">
                    <a class="navbar-brand" href="index.php">Wisata Yogyakarta</a><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1">
        ...
        <section id="borobudur" class="p-5">
            <div class="container-fluid d-flex flex-column align-items-center py-4 py-xl-5">
                <div class="row gy-4 row-cols-1 row-cols-md-2 w-100" style="max-width: 800px">
                    <?php
                    $sql = "SELECT *, image.id as id_image FROM image INNER JOIN candi
                        ON candi.id = image.id_candi
                        WHERE candi.nama = 'Candi Borobudur'";
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-img-overlay text-center d-flex flex-column justify-content-center align-items-center p-4">
                                        <form action="" method="POST">
                                            ...
                                            <button type="submit" name="delete_image_btn" class="btn btn-danger" onclick="return confirm('Apakah anda yakin ingin menghapus gambar ini ?')">Delete</button>
                                        </form>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="container d-flex flex-column align-items-center py-4 py-xl-5">
            <div class="row gy-4 row-cols-1 row-cols-md-2 w-100" style="max-width: 800px">
                ...
                $sql = "SELECT *, image.id as id_image FROM image INNER JOIN candi
                            ON candi.id = image.id_candi
                            WHERE candi.nama = 'Candi Prambanan'";
                $res = mysqli_query($conn,  $sql);
                            ...
                            <div class="card">
                                ...
                                <div class="card-img-overlay text-center d-flex flex-column justify-content-center align-items-center p-4">
                                    <form action=" " method="POST">
                                        <input type="hidden" name="delete_id" value="<?php echo $images['id_image']; ?>">
                                        <input type="hidden" name="delete_image" value="<?php echo $images['image']; ?>">
                                        <button type="submit" name="delete_image_btn" class="btn btn-danger">Delete</button>
                                    </form>
                                </div>
                            </div>
                        </div>
